...
create policy "Users can view own uploaded documents" on user_documents for select using (auth.uid() = user_id);
create policy "Users can insert own uploaded documents" on user_documents for insert with check (auth.uid() = user_id);
create policy "Users can update own uploaded documents" on user_documents for update using (auth.uid() = user_id);
create policy "Users can delete own uploaded documents" on user_documents for delete using (auth.uid() = user_id);
...
create policy "Service role can manage templates" on template_versions for all using (auth.jwt() ->> 'role' = 'service_role');
...
create index user_documents_industry_idx on user_documents(industry);
...
create index document_logs_status_idx on document_logs(status);
...
create index template_versions_created_at_idx on template_versions(created_at);
